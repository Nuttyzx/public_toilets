...
class Toilet {
  ...
  Toilet({ // contructor ชื่อเดียวกับ class
    required this.id,
    required this.name,
    //required this.point,
 ...
 // fromJson ตรงนี้ตั้งชื่อเป้นไรก้อได้
  ...
  factory Toilet.fromJson(Map<String, dynamic> json){ //รับค่า map ที่ได้จากการแปลงรูปแบบ json เป็น map แล้ว อยู่ในตัวแปรชื่อ json ลงใน object toilet
    ...
    json['reviews'].map<Review>((item) => Review.fromJson(item)).toList(); // แปลง list review ของ map  ไปเป็น list ของ object review
    ...
    return Toilet( // รีเทินค่าที่ได้เก็บข้อมูลที่รับมาใส่ลงในตัวแปร ของ model class toilet //return object toilet
      id: json['id'], // ต้องเขียนชื่อให้ตรงกับ api
      name: json['name'],
      /*point: json['rating'],*/
