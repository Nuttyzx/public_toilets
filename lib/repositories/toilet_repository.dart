...
class ToiletRepository { // class ที่ทำหน้าที่ ดึงข้อมูล จาก call api
  Future<List<Toilet>> getToilets() async {
    try { //call ไปที่ api caller ซึ่งมี method get ส่ง endpoint ไป
      var result = await ApiCaller().get('toilets?_embed=reviews'); //ออกสอบ ใส่ endpoint  //เรียก api จะได้ string ที่เป็น json
      ...
      list.map<Toilet>((item) => Toilet.fromJson(item)).toList(); // แปลงจากlist ของ object map เป็น list ของ object  toliet  // แปลง map แต่ละก้อนที่อยู่ใน list ไปเป็น object toilet ที่อยู่ใน list แทน
      return toiletList; //สิ่งที่ return จาก getToilets() ออกไป จะกลายเป็น list ของ toilet และใน toilet จะมี list ของ review ด้วย เอาไปใช้ใน ui ได้แล้ว
    } catch (e) {
      // TODO:
      rethrow;
    }
  ...
  Future<void> addToilet( // เพิ่มข้อมูลห้องน้ำ
      {required String name, required double distance}) async {
    try {
      var result = await ApiCaller() // call ไปที่ api caller ใช้ post
          .post('toilets', params: {'name': name, 'distance': distance}); // เตรียมพารามิเตอร์ 2 ตัว คือ name distance กรอกข้อมูล name distance
    } catch (e) {
      // TODO:
      rethrow;
    }
