...
class _AddToiletPageState extends State<AddToiletPage> {
  ...
  validateForm() { // ฟอมที่กรอก 2 ช่อง ต้องไม่ว่าง
  ...
  saveToilet() async {
    ...
    try {
      ...
      var distance = double.parse(_distanceController.text); // แปลงจาก text เปน double

      await ToiletRepository().addToilet(name: toiletName, distance: distance); //call addToilet ที่อยู่ใน ToiletRepository

      if (mounted) Navigator.pop(context); // pop ลูกศรย้อนกลับไปยังหน้าเก่า ปุ่ม back
    } catch (e) {
      setState(() {
